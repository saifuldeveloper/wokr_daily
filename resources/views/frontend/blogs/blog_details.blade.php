    ...
    <script>
        $(document).ready(function (){
            @if(auth()->check() && $blog->comments_count > 0)
                getComments();
            @endif
            $(document).on('click','.show_more_replies',function (){
                let comment_id = $(this).data('comment_id');
                $('.comments-template.comments-template-v2 .comments-list.active .comment .reply_for_'+comment_id+' ul.children li').show();
                $(this).hide();
            })
        })
