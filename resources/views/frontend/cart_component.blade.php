        ...
        <div class="cart-product" id="cart_{{$course->id}}">
            ...
            <div class="content">
                ...
                <div class="quantity-price-wrapper">
                    ...
                    <span class="price">{{ $course->is_free ? __('free') : get_price($course->price) }}</span>
                </div>
            </div>
        ...
        <div class="cart-product" id="cart_{{$book->id}}">
            ...
            <div class="content">
                ...
                <div class="quantity-price-wrapper">
                    <span class="quantity">1 ×</span>
                    <span
                        class="price">{{ $book->is_free ? __('free') : get_price($book->price) }}</span>
                </div>
            </div>
...
<div class="cart-footer">
    <div class="subtotal-wrapper">
        ...
        <span class="subtotal">{{ get_price($sub_total) }}</span>
    </div>
    <ul class="cart-btns">
        <li><a href="{{ route('cart.view') }}" class="template-btn">{{__('view_cart')}}</a></li>

        @if (intval(substr(get_price($sub_total),1)) > 0)
