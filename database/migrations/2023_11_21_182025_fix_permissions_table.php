...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        $permission = Permission::where('attribute', 'cache setting')->first();

        $permissions = $permission->keywords;
        foreach ($permissions as $key=> $keyword) {
            if ($keyword == 'admin.cache') {
        ...
        $permission = Permission::where('attribute', 'firebase setting')->first();

        $permissions = $permission->keywords;
        foreach ($permissions as $key=> $keyword) {
            if ($keyword == 'admin.firebase') {
                unset($permissions[$key]);
            }
        }
        $permission->update([
        ...
        $permission = Permission::where('attribute', 'miscellaneous setting')->first();

        $permissions = $permission->keywords;
        foreach ($permissions as $key=> $keyword) {
            if ($keyword == 'miscellaneous.setting') {
                unset($permissions[$key]);
            }
        }
        $permission->update([
        ...
        $permission = Permission::where('attribute', 'admin panel setting')->first();

        $permissions = $permission->keywords;
        foreach ($permissions as $key=> $keyword) {
            if ($keyword == 'admin.panel-setting') {
                unset($permissions[$key]);
            }
        }
        $permission->update([
